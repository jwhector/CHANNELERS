...
- **`apps/stage`** — Vite/React; one app, role-based routes. Station screens (`/intake`, `/bodyscan`, `/altar`) gate on **confirm-at-station**, not a typed number (`/intake` & `/altar` via `CalledGate`; `/bodyscan` renders straight from `dispatch.state`). `/intake` is visitor self-confirm; `/bodyscan` & `/altar` are **performer-confirmed** (a guide admits the visitor from `/station`). Routes:
  ...
  - `/perform` — one-device performer console: `altar` (camera-less) · `channel` · `choreo` as tabs, **reusing the standalone components**. All three stay mounted (inactive tabs `hidden`) so each child's session/socket/audio survives switching and choreo ear-audio keeps playing. The altar tab is camera-less (`<Altar showCamera={false}>`); standalone `/altar` keeps the camera as an opt-in fallback.
  - `/console` — master overseer (visitors / flow funnel / sessions) · Pluribus altar-ready broadcast (TTS, output picker).
  - `/board` — public roster as a bare white-phosphor PC terminal (VT323; every visitor number + current location, `called` = inverse-video now-serving; a waiting-and-unplaced visitor shows `WAITING ROOM`/`ON HOLD` via the pure `boardRows()`, #24) · `/dispatch` — lobby-operator queue board (+ flow strip: altar CLOSED/OPEN gate, altar-ready count, bodyscan idle/blocked; right column lists altar-ready visitors; + Pluribus altar-ready broadcast control) · `/souvenir` — QR takeaway.

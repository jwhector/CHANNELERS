...
- **`apps/stage`** — Vite/React; one app, role-based routes. Station screens (`/intake`, `/bodyscan`, `/altar`) gate on **confirm-at-station**, not a typed number (`/intake` & `/altar` via `CalledGate`; `/bodyscan` renders straight from `dispatch.state`). `/intake` is visitor self-confirm; `/altar` is **operator-confirmed on its own surface** (the altar performer admits/repools inline — no `/station` screen); `/bodyscan` is **performer-confirmed** (a guide admits the visitor from `/station/bodyscan`). Routes:
  - `/intake` · `/altar` — visitor stations (survey · pose verify + persona). `/altar` is the altar performer's surface: when a visitor is called it shows **Confirm arrival + Release** (`CalledGate confirmedBy="operator"`), then **defaults to override** for the pose (`Unlock (override)` is the primary action; the camera pose-match is an opt-in `verify by camera` fallback) before the persona pick. `/intake` uses the CRT skin (`CrtShell`).
  ...
  - `/console` — master overseer (visitors / flow funnel / sessions) · **altar open/close gate** (`AltarGate` → `POST /api/dispatch/altar`) · Pluribus altar-ready broadcast (TTS, output picker, **speed dial** — `SpeedPicker`/`rate.console`, as on `/choreo` + `/channel`). The altar gate + broadcast moved here off `/dispatch`.

...
- **Next up:** **#18 (ALTAR READY on `/board`)** to finish the legibility batch (small; uses existing `isAltarReady`), then **#10 (CRT resolution/overscan)** and **#8 (Rachel's form, still blocked on content)** to close Stream C. Otherwise **#22 (mimic)** once a repro is in hand (resume systematic-debugging from the captured notes — brain logic looked correct, so suspect a live/multi-surface or sequence-specific condition), or Stream D (XP skin). **Eyeball #7/#9 on the CRT rig** when convenient (font sizes + exit directive are CSS, not asserted in jsdom). **Live-verify Stream E on devices:** open `/perform`, confirm tab switching keeps choreo audio playing and a claimed channel session survives switching.

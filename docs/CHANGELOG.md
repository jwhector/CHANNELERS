...
- **Why:** Jared's creative direction for the "into the matrix" moment — words readable one at a time before turning to binary, standardized kerning so nothing jumps, full text sized dynamically to fit.
...
- **Verification:** stage typecheck 0 errors; build OK; stage **41** tests pass (14 files). **Not run here:** the in-browser visual smoke (timing/fit feel) — tunable via `DEFAULT_KNOBS` in `paperAnim.ts` (and the matching `1.5s` fade in `feed.css`).

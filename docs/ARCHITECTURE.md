...
channelers/
  apps/
    ...
    stage/      Vite + React + TypeScript                     — all screens, role-based routes:
                  ...
                  /station   per-station performer arrival-confirm view (bodyscan/paper; altar admits on /altar, intake self-confirms) (§5.x)
                  ...
                  -- waitingroom was retired as a station (#24): it's now a board-derived holding bucket, not a dispatched slot. The physical hourglass is a stage direction.
  packages/
...
**Selection:** anti-starvation over random among eligible `waiting` visitors not already occupying a slot (eligibility: intake ← missing `intakeAt`; bodyscan ← missing `poseAt`; altar ← `intakeAt` + `poseAt` set, no `sessionEndAt`, **and the operator altar gate is open**; paper ← missing `paperAt`), `fill()`ing only **free online** slots, gated by:
- **Fill priority:** `fill()` serves free slots in `fillPriority` order (default **bodyscan → intake → altar → paper**) so the scarce single bodyscan gate gets first pick of the waiting pool each pass — it can't lose its only candidate to the 2-wide intake or to a soak. (Selection *within* a station is unchanged: anti-starvation over random.)
...
|------|---------|---------|---------|
| `fillPriority` | — | `[bodyscan, intake, altar, paper]` | Order `fill()` serves free slots — scarce gate (bodyscan) first, soaks last |
...
### 5.7 Kiosk-less group stations — Scan/Shred/Feed (`/feed`)

A second station **kind**: a kiosk-less group station, the (now only) instance being `paper` / `/feed` (spec `docs/superpowers/specs/2026-06-22-group-stations-design.md`; open questions in §12). It reuses the dispatcher but inverts its assumptions:

- **Dispatcher kind (`config.dispatcher.groupStations`).** A group station's slots are **always online** (no kiosk binding), capacity = `config.dispatcher.slots.paper` (group capacity, default 3), and eligibility is non-gating (`!paperAt`). A group occupant flows through the same lifecycle as a kiosk one — `called → (performer confirms arrival at /station/paper) → in_progress → (performer taps Done) → markComplete stamps paperAt`. **Checkout is manual** (rehearsal #17): `reconcile()` never auto-completes or stale-reaps an in-progress group occupant — its only exit is **Done** (`POST /api/dispatch/complete`). **No-show still applies while `called`** (a participant called to `/feed` who never arrives is repooled/flagged). A generic **dwell** capability is retained (`config.dispatcher.timed[station].dwellMs` → auto-complete + a stale backstop + a `/dispatch`/`/station` "Ns left" countdown via `timedDwellMs`) for a hypothetical future timed station, but **no station is configured timed** (`timed: {}`); `paper` is deliberately manual.

...
- The Brain is now planned to run in the **cloud**, a deviation from the documented "**local** Show Brain" (§2/§8). This ripples beyond Ableton: the stage screens' WS connection, the live-oracle latency budget, and offline-resilience all assume a venue-local Brain. **Open for the team:** is cloud-Brain the committed deployment, or does the Brain stay local at the venue? (Answer shapes week-2 networking + the latency budget.)

...
channelers/
  apps/
    ...
    stage/      Vite + React + TypeScript                     — all screens, role-based routes:
                  ...
                  /altar     altar performer's surface: admit (Confirm arrival/Release) → pose verify + persona pick → oracle-ready (verify defaults to override; camera is an opt-in fallback)
                  ...
                  /perform   one-device performer console: altar (camera-less) · channel · choreo · console as tabs; reuses the standalone components (all mounted, inactive ones hidden so sessions/sockets/audio persist)
                  /console   master overseer: visitors+controls / flow funnel+station LEDs / altar gate + Pluribus broadcast / sessions+event log
                  /board     public call display: #N → STATION, or WAITING/ON HOLD/ALTAR READY for the waiting-and-unplaced (live dispatch.state broadcast)
                  /dispatch  lobby-operator interface: register arrivals, confirm/skip calls, manage queue
                  /station   per-station performer arrival-confirm view (bodyscan/paper/offering; altar admits on /altar, intake self-confirms) (§5.x)
                  /feed      Scan/Shred/Feed — the paper group station's spectacle screen (§5.7)
                  /souvenir  QR takeaway
                  -- paper is a kiosk-less group station with MANUAL checkout (no dwell); a performer ends the visit with Done at /station/paper (#17).
                  -- offering is the "time offering" room: a kiosk-less TIMED station (dwell auto-release, default 5 min) with a manual early Done at /station/offering. The lobby-overflow board label is "WAITING"; the room itself shows STATION A - TIME OFFERING via its slot.
  packages/
...
**Two confirms (spec §5):** arrival is an explicit press, not a typed number. (1) **Confirm call** — lobby operator on `/dispatch` (`pending → called`, pinned to the slot; skipped when `autoConfirm`). (2) **Confirm arrival** (`called → in_progress`, `POST /api/dispatch/arrive`) — **who** presses it depends on the station: `/intake` is **visitor self-confirm** (the participant presses the existential **I AM** themselves — a fully digital station); **`/altar` is operator-confirmed on its own surface** — `CalledGate confirmedBy="operator"` renders **Confirm arrival + Release** inline on `/altar`, so the altar performer admits (or repools) the called visitor without a separate `/station` screen; **`/bodyscan`** renders straight from `dispatch.state` and is admitted by the guide from the `/station/bodyscan` view. So the `/station/:station` admit view now covers **bodyscan + paper only**. `CalledGate`'s `confirmedBy` is `"visitor"` (default, self-confirm button) | `"operator"` (Confirm arrival + Release); arrival is auto-confirmed when `autoArrive`. `CalledGate` also takes `skin?: "crt" | "default"`: `/intake` opts into the **CRT skin** (amber segmented-LED number + the **I AM** confirm, inside `CrtShell`); the others keep the default gate. The `/station` view is **passive** — it reads `dispatch.state` and POSTs `arrive`/`repool`, sends no `station.hello`, and so never claims a slot or disturbs kiosk binding. The whole `/intake` flow (standby · called · survey · processed) is skinned as a DMV-purgatory CRT terminal — see the Intake CRT redesign in `CHANGELOG.md`.

**Selection:** anti-starvation over random among eligible `waiting` visitors not already occupying a slot (eligibility: intake ← missing `intakeAt`; bodyscan ← missing `poseAt`; altar ← **ALL pre-altar stations done** (`intakeAt` + `poseAt` + `paperAt` + `offeringAt`, via the shared `clearedPreAltarStations`), no `sessionEndAt`, **and the operator altar gate is open**; paper ← missing `paperAt`; offering ← missing `offeringAt`), `fill()`ing only **free online** slots, gated by:
- **Fill priority:** `fill()` serves free slots in `fillPriority` order (default **bodyscan → intake → altar → paper → offering**) so the scarce single bodyscan gate gets first pick of the waiting pool each pass — it can't lose its only candidate to the 2-wide intake or to a soak. (Selection *within* a station is unchanged: anti-starvation over random.)
- **Altar gate:** `altarOpen` (in-memory, **default closed**) gates altar eligibility — the altar is a deferred/batch event, so no visitor is dispatched to it until the operator opens it on `/console` (`POST /api/dispatch/altar { open }`). In-progress readings are unaffected (eligibility only touches `waiting` visitors). Opening it `kick()`s an immediate fill.
- **Holds:** a visitor is ineligible for a new slot assignment until `max(createdAt + introHoldMs, no-show cooldown)`. Two sources feed one `isHeld(v)` check: the **intro hold** (derived from `createdAt`, no extra storage) staggers each fresh registrant; the **no-show hold** (a small per-visitor map, set at `noShowMs` timeout) gates a repooled no-show for `noShowHoldMs`. Anti-starvation skips held visitors — they cannot be priority-rescued while held. Held visitors stay in `queue` (with `heldUntil?`/`holdReason?`) so the UI can show countdowns. Manual `assign()`/`checkin()` bypass holds.
...
**No-show:** a visitor called but not arrived after `noShowMs` is flagged `no-show`. Operator can re-pool manually; the `noShowAutoRepool` knob auto-re-pools instead. After a no-show the visitor is held for `noShowHoldMs` before becoming eligible again (see Holds above). `/station` shows each called row's countdown to no-show.

**Knobs** (all in `config.dispatcher`, env-overridable):

| Knob | Env var | Default | Meaning |
|------|---------|---------|---------|
| `fillPriority` | — | `[bodyscan, intake, altar, paper, offering]` | Order `fill()` serves free slots — scarce gate (bodyscan) first, soaks last |
| `groupStations` | — | `[paper]` | Kiosk-less always-online stations with no hardware binding; exit by manual Done unless also in `timed` (#17) |
| `timed.offering.dwellMs` | `OFFERING_DWELL_MS` | 300 000 | The "time offering" room's timed auto-release (manual early Done also frees it) |
...
| `autoArrive` | `DISPATCH_AUTO_ARRIVE` | false | Skip performer arrival step (called → in_progress) — dev/stub |
| `noShowAutoRepool` | `DISPATCH_NOSHOW_AUTOREPOOL` | false | Auto-re-pool no-shows |

Defaults are rehearsal-fast (short timers). Tune for a full-audience show via env. For hands-free local runs set `DISPATCH_AUTO_CONFIRM=true` **and** `DISPATCH_AUTO_ARRIVE=true` (no operator/performer needed).

**Transport:** a `dispatch.state` WS broadcast carries the full `DispatchState` — `{ slots: Slot[], queue: DispatchQueueEntry[], completed, surplus, stationsOnline, timedDwellMs?, noShowMs?, altarOpen, altarReady, bodyscanIdle, bodyscanBlocked }` (the slots array replaces the old per-station count map + call board; `timedDwellMs` carries the per-timed-station dwell so the board can show a countdown, §5.7; `noShowMs` is the called-but-not-arrived threshold so `/station` can show a per-row countdown; `altarOpen` is the operator altar gate; the derived **flow-health** fields — `altarReady` (count of waiting visitors who cleared ALL pre-altar stations), `bodyscanIdle`, and `bodyscanBlocked: "none"|"soaking"|"held"|"empty"` — let `/dispatch` show the altar-ready buffer and why the scarce gate is starved; each `occupant` may carry its own review `flags`; each `queue` entry may carry `heldUntil?` (epoch-ISO) and `holdReason?: "intro" | "no-show"` so `/dispatch` can render a live hold chip; `completed` = visitors with `sessionEndAt`; `surplus` = connected screens with no free slot; `stationsOnline` = derived per-station LED; `warmedUp` removed). It is pushed on every state change **and** sent to each screen on connect. This channel is **screens-only — never OSC** — dispatcher logistics are deliberately kept off the `ShowEvent`/OSC contract (§9-ext). `/dispatch` renders this as a no-scroll 3-zone board (waiting pool · slot grid · completed) plus a **flow strip** (altar-ready count, bodyscan idle/blocked readout); `/board` derives the public call list from slots in the `called` phase; `/console` reads the slots array, keeps the manual override, and owns the **altar open/close gate** (`POST /api/dispatch/altar`) and the **Pluribus altar-ready broadcast** (both moved off `/dispatch`).
...
### 5.7 Kiosk-less group stations — Scan/Shred/Feed (`/feed`) + the Time Offering room

A second station **kind**: a kiosk-less group station — slots that are **always online** with no kiosk binding (spec `docs/superpowers/specs/2026-06-22-group-stations-design.md`; open questions in §12). Two instances exist, split by how they complete: **`paper`** / `/feed` (manual checkout) and **`offering`** (the timed "time offering" room). Both reuse the dispatcher but invert its kiosk assumptions.

- **Dispatcher kind (`config.dispatcher.groupStations` + `config.dispatcher.timed`).** A group occupant flows through the same lifecycle as a kiosk one — `called → (performer confirms arrival at /station/<station>) → in_progress → completion`. Always-online is driven by `isOnline = isGroup(s) || isTimed(s) || !!connId`. **How it completes depends on config:**
  - **`paper` — manual (`groupStations`).** Capacity = `slots.paper` (default 3), eligibility non-gating (`!paperAt`). `reconcile()` never auto-completes or stale-reaps it — its only exit is **Done** (`markComplete`, `POST /api/dispatch/complete`) which stamps `paperAt` (rehearsal #17).
  - **`offering` — timed + manual-early (`timed`).** The "time offering" room: capacity = `slots.offering` (default 5), eligibility non-gating (`!offeringAt`), and it **auto-releases on a dwell** — `reconcile()`'s timed branch stamps `offeringAt` and frees the slot once `timed.offering.dwellMs` (`OFFERING_DWELL_MS`, default 5 min) elapses from arrival. A performer can **release early** with the same **Done**/`markComplete`. `/dispatch` + `/station` show the "Ns left" countdown from `timedDwellMs`. (A station may be in *both* sets; the timed branch wins, so being in `timed` is what makes `offering` auto-complete.)
  - **No-show still applies while `called`** for both (a participant called who never arrives is repooled/flagged, never falsely completed).
...
- **Time Offering room — `offering` (no own screen).** A held room where a visitor "offers their time" for a fixed dwell, performer-confirmed at `/station/offering` (no spectacle screen, unlike `/feed`). It is a timed group station as above (5 slots, 5-min dwell, milestone `offeringAt`, label `STATION A - TIME OFFERING`), with the **manual early Done** as the operator's release valve. History: a `waitingroom` station was retired in #24 (reframed as a pure board label); this re-adds the room as a *new* `offering` id pointed at the kept timed machinery rather than a literal revert. **Board:** an `offering` occupant shows as `STATION A - TIME OFFERING` via its slot; the separate lobby-overflow label for waiting-and-unplaced visitors is `WAITING`/`ON HOLD`, with the altar-ready subset `ALTAR READY` (#18) — all derived by the pure `boardRows()` (unioning `queue` + `altarReadyList`). The physical hourglass is a stage direction.
...
- **TTS:** the oracle's line is spoken via **ElevenLabs** (`ELEVENLABS_MODEL`, default `eleven_flash_v2_5` — low-latency, intelligibility > character in earpiece/whisper mode), proxied through the brain's `/api/tts` so the key stays server-side. **Voices are per-archetype**, mapped as `voiceId` on each persona in `packages/oracles` (stock premade voices for now — retune freely). **Fallback chain (each step keeps audio *routable*):** ElevenLabs → **OpenAI TTS** (`OPENAI_TTS_MODEL`, default `gpt-4o-mini-tts`, neutral voice; used when `ELEVENLABS_API_KEY` is unset but `OPENAI_API_KEY` is) → (no keys) the browser's `speechSynthesis`. **Per-device output routing** rides `HTMLMediaElement.setSinkId` on the MP3 path only — so the central-machine + Scarlett 8i6 per-performer in-ear setup works whenever *either* TTS key is present; `speechSynthesis` always plays to the system default and is flagged "default only" in the UI. The performer's output device is chosen on `/channel` (and `/choreo`) via a per-tab picker (`?out=` override); see §12.
...
dispatch.state  { slots: Slot[], queue, completed, surplus, stationsOnline, timedDwellMs?, noShowMs? }   dispatcher snapshot (screens only; queue entries carry heldUntil?/holdReason?)
...
The `dispatch.state` message is broadcast on every dispatcher state change **and** sent on connect. It carries the full `DispatchState`: the **addressable slot array** (`slots: Slot[]`, each with `id`, `station`, `kioskId?`, `online`, and a pinned `occupant?` of phase `pending`/`called`/`in_progress`), the waiting `queue` (entries carry `heldUntil?`/`holdReason?` for live hold countdowns on `/dispatch`), the `completed` list (`sessionEndAt` set), `surplus` screens (connected but unbound), the derived `stationsOnline` LEDs, and `noShowMs` (so `/station` can render per-row countdown). `warmedUp` removed. Arrival is an explicit **Confirm arrival** at the kiosk (`POST /api/dispatch/arrive`), not a typed check-in.
...
**Venue & hardware**
- How many tablets/kiosks for intake? A projector for Jeff? (**Scan-station webcams** ✅ **RESOLVED for MVP (2026-06-22):** `/bodyscan` + `/altar` have a per-tab **camera picker** (`getUserMedia` `deviceId`, `?cam=` override), so a high-quality **iPhone via Continuity Camera** on a Mac station can be selected — directly helps the §6 full-body-framing/lighting risk. Tablet kiosks fall back to their own camera.)
...
- ~~**How audio reaches the earpiece / per-performer routing**~~ — ✅ **RESOLVED for MVP (2026-06-22):** two supported topologies. (a) **Per-performer device** — each performer runs `/channel` on their own phone/tablet + earbuds (output = that device; scales trivially, matches cloud/5G resilience). (b) **Central machine + Focusrite Scarlett 8i6** — one box runs N windows, each routed to a different output via a per-tab picker (`HTMLMediaElement.setSinkId`, `?out=` override) → discrete Scarlett outputs → wired IEM feeds (~6 mono feeds; split the interface into per-pair virtual devices with Loopback/Aggregate). Routing requires the MP3 path, so the §7 **OpenAI-TTS fallback** keeps it working without an ElevenLabs key. Still open: the physical receiver brand/model + tolerable latency (rig-dependent).
- How many performers need a simultaneous feed? (Output count is bounded by the chosen topology — the 8i6 caps the central-machine path at ~6.)
...
**Deployment — cloud Brain + the Ableton bridge (2026-06-22)** — *new this session*
- The Brain runs in the **cloud**, a deviation from the documented "**local** Show Brain" (§2/§8) — **decided 2026-06-22** (Jared: everything remotely accessible). This ripples beyond Ableton: the stage screens' WS connection, the live-oracle latency budget, and offline-resilience all assumed a venue-local Brain. Driver: the show is at a university where LAN peer-to-peer is often blocked by WiFi client-isolation, while outbound 443 is reliable — so every screen reaching a public HTTPS origin is more robust than discovering a venue-local Brain. (5G-hotspot fallback works identically since there's no LAN dependency.)
- **Single-origin deployment is now built (2026-06-22).** The Brain serves the stage's Vite build alongside `/api` + `/ws` from one HTTPS origin (`@fastify/static` + SPA fallback, env `SERVE_STAGE`; `apps/brain/src/app.ts`), matching the stage's hard-relative `/api` + `/ws` (no configurable base URL). A WS **keepalive** ping (`apps/brain/src/bus.ts`, `WS_HEARTBEAT_MS` default 30s) keeps idle sockets warm through proxy/NAT timeouts and reaps half-open ones. Ships `Dockerfile` + `fly.toml` + `DEPLOY.md` (one always-on machine — the store is in memory, so no scale-to-zero / no horizontal scale; `force_https` → wss). **Open / next:** pick the Fly region near the venue; a venue-side **OSC relay for Anna/Jeff** (generic OSC out, mirroring the Ableton dial-home — cloud can't UDP into the venue LAN); state across redeploys is now **recoverable but not transactional** — an opt-in visitor-store snapshot (`VISITOR_SNAPSHOT_PATH` → a mounted Fly volume, 2 s cadence, hydrate-on-boot; built 2026-06-28) restores participant data (surveys/poses/archetypes/milestones) after a crash/redeploy, while dispatcher slot positions and live divination sessions still re-derive on reconnect; avoid redeploying mid-show, but a crash is no longer total data loss.
- For Ableton control under a cloud Brain, a standalone, **reusable** `ableton-osc-bridge` package was built — deliberately decoupled from CHANNELERS (`app/packages/ableton-osc-bridge/`, zero `@channelers/*` imports; spec `docs/superpowers/specs/2026-06-22-ableton-osc-bridge-design.md`). A venue-side daemon speaks UDP-OSC to AbletonOSC on the LAN and **dials home** over one authenticated `wss` to the Brain, so raw OSC never crosses the internet. It exposes a comprehensive typed facade (`createLive(provider)`) usable identically from the Brain (over the network client) or a same-machine backend.
- **Brain↔bridge wiring is now done (Plan C, 2026-06-22).** The dial-home topology is complete on both ends: the package added `createBridgeHost()` (`ableton-osc-bridge/host`) — the controller that accepts the daemon's inbound socket and exposes a stable typed `Live` — and the Brain arms it as a **token-gated `/agent` WS endpoint** (`apps/brain/src/ableton.ts`, env `ABLETON_AGENT_TOKEN`, **off by default**; `getLive()` returns the facade). The daemon was unchanged (it already dials home); proven end-to-end by a loopback test using the real `dialHome()`. Stays off the §8 `/channelers/*` show-event contract. **Open / next:** the **event→Ableton mapping** — which show moments should drive Live (the remaining creative step) — and the cloud-deployment decision above.

**Group/timed stations (2026-06-22)** — from `docs/superpowers/specs/2026-06-22-group-stations-design.md` §15. *Built 2026-06-22 (branch `group-timed-stations`); these remain open for the team.*
- **Typewriter sequencing** — must `paper` gate on the visitor having produced a typed page (a future `typedAt` milestone), or stay **ungated** as in the MVP? Is the upstream typewriter dispatcher-managed at all, or a purely physical corner that just hands out pages?
- **Station #2** — still undefined; the timed-group backbone is built to host it later with config + one eligibility line (do not invent it yet).
- **Knob tuning** — `slots.paper` (group capacity, default 4) and `paperDwellMs` (dwell, default 5 min, operator-only countdown) — confirm they feel right at audience scale in rehearsal.
- **Feed identity upgrade** — if fed text should ever be ingested per-visitor (music seed / oracle / collective wall), add `visitorId?` to `paper.fed` and decide the at-the-slot identity mechanism then (non-breaking additive change).
- **Shred** — promote to system-registered (`paper.shredded` event + a "destruction" spectacle, *"even destroying your confession is logged"*) later, or leave physical-only?
- **Physical** — webcam framing over the feed slot; button hardware (USB arcade button / footswitch presenting as a keypress); whether capture and display are one screen or split (`?role=capture` / `?role=display`).

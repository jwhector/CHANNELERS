...
- **Altar gate:** `altarOpen` (in-memory, **default closed**) gates altar eligibility — the altar is a deferred/batch event, so no visitor is dispatched to it until the operator opens it on `/dispatch` (`POST /api/dispatch/altar { open }`). In-progress readings are unaffected (eligibility only touches `waiting` visitors). Opening it `kick()`s an immediate fill.

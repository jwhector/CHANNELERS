...
**Selection:** anti-starvation over random among eligible `waiting` visitors not already occupying a slot (eligibility: intake ← missing `intakeAt`; bodyscan ← missing `poseAt`; altar ← `intakeAt` + `poseAt` set, no `sessionEndAt`, **and the operator altar gate is open**; paper ← missing `paperAt`; offering ← missing `offeringAt`), `fill()`ing only **free online** slots, gated by:

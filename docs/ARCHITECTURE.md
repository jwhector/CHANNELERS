...
**Selection:** anti-starvation over random among eligible `waiting` visitors not already occupying a slot (eligibility: intake ← missing `intakeAt`; bodyscan ← missing `poseAt`; altar ← `intakeAt` + `poseAt` set, no `sessionEndAt`, **and the operator altar gate is open**; paper ← missing `paperAt`; waitingroom ← missing `waitingRoomAt`), `fill()`ing only **free online** slots, gated by:
- **Fill priority:** `fill()` serves free slots in `fillPriority` order (default **bodyscan → intake → altar → paper → waitingroom**) so the scarce single bodyscan gate gets first pick of the waiting pool each pass — it can't lose its only candidate to the 2-wide intake or to a 5-min soak. (Selection *within* a station is unchanged: anti-starvation over random.)
